...
def load_config():
    ...
    config.read("C:/Users/vlad/Desktop/ПС_питон/HW3_11/settings.ini")
    return {
...
def create_menu(root):
    ...
    menu_bar = tk.Menu(root, bg=config["bg_color"])
    ...
    graphic_menu.add_command(label="Тип оплаты по категориям клиентов", command=type_of_pay)
    graphic_menu.add_command(label="Распределение заказов по часам", command=orders_by_hour)
    graphic_menu.add_command(label="Разброс цен по категориям блюд", command=price_boxplot)
    graphic_menu.add_command(label="Распределение суммы заказа по времени и типам клиентов", command=sum_scatter)

    #Иниициализируем пункты меню текстовых отчетов
    text_menu.add_command(label="Количество заказов по часам", command=orders_by_hour_text)
    text_menu.add_command(label="Количество заказов по кассирам", command=orders_by_cashiers)
    text_menu.add_command(label="Количество блюд разных категорий по часам", command=pivot_analyze)

    #Иниициализируем пункты меню баз данных
    databases.add_command(label="Кассиры", command=cashiers_database)
    databases.add_command(label="Типы клиентов", command=client_types_database)
    databases.add_command(label="Меню", command=menu_database)
    databases.add_command(label="Состав заказов", command=order_database)
    databases.add_command(label="Заказы", command=orders_database)

    #Инициализируем создание справки
    app_info.add_command(label="Справка", command=info)
    ...
    menu_bar.add_cascade(label="О приложении", menu=app_info)
